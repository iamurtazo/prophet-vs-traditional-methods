...
1. Extend comparison to include ARIMA, LSTM, and other machine learning methods
2. Apply methodology to actual demand data (not just prices)
3. Investigate hybrid approaches combining Prophet with domain-specific regressors
4. Develop integration frameworks for ERP/planning systems
5. Study change management aspects of transitioning from Excel to Prophet
...
# Appendix: Excel Methodology

## A.1 Simple Moving Average (SMA) in Excel
...
## A.2 Exponential Moving Average (EMA) in Excel
...
Cell D2: =B2
Cell D3: =B3*$E$1 + D2*(1-$E$1)
... (drag down)
```

## A.3 Linear Trend Analysis in Excel

### Method 1: TREND Function

```
=TREND(known_y's, known_x's, new_x's)

Example:
=TREND(B2:B100, A2:A100, A101:A130)
```
...
Example (forecast for row 101):
=FORECAST(A101, B2:B100, A2:A100)
```

### Method 3: Manual Regression

1. Calculate slope: `=SLOPE(B2:B100, A2:A100)`
2. Calculate intercept: `=INTERCEPT(B2:B100, A2:A100)`
3. Forecast: `=Slope * Future_Period + Intercept`

## A.4 Holt-Winters in Excel
...
Row 2:
E2: =B2 (initial level)
F2: =B3-B2 (initial trend)

Row 3+:
E3: =$G$1*B3+(1-$G$1)*(E2+F2)
F3: =$G$2*(E3-E2)+(1-$G$2)*F2
... (drag down)
